...
def get_smart_indices_from_folder(folder_path):
    ...
    # PARAMETERS
    # =====================
    #[40, 4, 6, 0.3, 0.2], [20, 2, 3, 0.3, 0.4], [10, 2, 3, 0.3, 0.5], [15, 2, 3, 0.3, 0.3]
...
# # --- Configuration ---
# folder_in = "/mnt/disk4/video-panninng-classification/datasets/frames_v3"
# csv_path = '/mnt/disk4/video-panninng-classification/datasets/csv/train.csv'
# checkpoint_path = csv_path.replace('.csv', '_checkpoint.csv') # e.g. train_checkpoint.csv
# output_column = 'selected_frames_new_new_new_new_new'

# def process_row(row_data):
#     """
#     Your actual processing logic goes here.
#     Returns the processed value (string) to be saved.
#     """
#     try:
#         # Example: Extract the video path or ID from the row
#         # video_path = row_data.get('video_path') 
#         # result = do_heavy_calculation(video_path)
        
#         # --- PLACEHOLDER FOR YOUR LOGIC ---
#         import time
#         time.sleep(0.1) # Simulating work
#         return "processed_value_example" 
#         # ----------------------------------
...
#     """Reads the checkpoint file into a dictionary for fast lookups."""
#     cache = {}
...
#             for line in reader:
#                 if len(line) >= 2:
#                     # Assuming column 0 is ID (video path) and column 1 is the result
...
#     # Ensure output column exists in headers
#     if output_column not in fieldnames:
#         fieldnames.append(output_column)

#     # 2. Load existing progress from Checkpoint File
#     # We use a unique identifier (like 'video_path' or 'id') to map results.
#     # CHANGE 'video_path' below to the actual unique column name in your CSV
#     unique_id_col = fieldnames[0] # Defaulting to first column if unsure, usually 'video_path'
    
#     processed_cache = load_checkpoints()
#     print(f"Found {len(processed_cache)} rows already processed in checkpoint.")

#     # 3. Identify rows that need processing
#     rows_to_process = []
#     indices_to_process = []

#     for i, row in enumerate(rows):
#         unique_key = row[unique_id_col]
        
#         # Check if we already have it in the checkpoint
#         if unique_key in processed_cache:
#             row[output_column] = processed_cache[unique_key]
        
#         # Check if the column is empty or needs processing
#         if not row.get(output_column) or row[output_column] == "":
...
#     else:
#         # 4. Open Checkpoint file in APPEND mode
#         # We assume the checkpoint format is: unique_id, result_value
#         with open(checkpoint_path, 'a', newline='', encoding='utf-8') as chk_file:
#             chk_writer = csv.writer(chk_file)

#             # 5. Parallel Processing
#             with ProcessPoolExecutor() as executor:
#                 # Submit all tasks
#                 # We map the Future object to the row index so we know which row it belongs to
#                 future_to_index = {
#                     executor.submit(process_row, row): idx 
#                     for idx, row in zip(indices_to_process, rows_to_process)
#                 }

#                 # Process as they finish (Immediate Update)
#                 pbar = tqdm(total=len(rows_to_process), desc="Processing")
...
#                         # A. Update in-memory list
#                         row[output_column] = result
                        
#                         # B. Write to Checkpoint immediately
#                         chk_writer.writerow([unique_key, result])
#                         chk_file.flush() # Ensure it hits the disk immediately
                        
#                     except Exception as exc:
#                         print(f"Row {original_idx} generated an exception: {exc}")
                    
#                     pbar.update(1)
                
#                 pbar.close()

#     # 6. Final Write: Update the Main CSV
#     # Only done if we finish successfully or decide to stop
...
#     # Optional: Delete checkpoint if 100% complete
#     # if os.path.exists(checkpoint_path):
